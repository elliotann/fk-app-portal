...
class action {
    ...
    onInit = ({ component, injections }) => {
        ...
        //history增加
        history.listen('fk-app-portal', this.listen)
    }
    ...
    componentWillUnmount = () => {
        history.unlisten('fk-app-portal', this.listen)
    }


    load = async () => {
        //网站中不存在login应用，那么就不做用户相关处理，正式环境应该不需要这段代码，仅为单应用运行使用
        if (!this.config.apps['fk-app-login']) {
            const user = { id: 1, mobile: '15622887224', nickname: 'andy' }
